...
const printPlainError = err => {
  console.error('Error starting mlchain')
  console.error(err)
...
try {
  ...
  process.MLCHAIN_VERSION = metadataContent.version
  process.BPFS_STORAGE = process.core_env.BPFS_STORAGE || 'disk'

  const configPath = path.join(process.PROJECT_LOCATION, '/data/global/mlchain.config.json')
  ...
  require('yargs')
    .command(
      ...
      'Start your mlchain server',
      {
        ...
        autoMigrate: {
          description:
            'When this flag is set, Mlchain will automatically migrate your content and configuration files when upgrading',
          default: false,
          type: 'boolean'
        }
      },
      argv => {
        ...
        process.TELEMETRY_URL = process.env.TELEMETRY_URL || 'https://telemetry.mlchain.cloud/ingest'
    ...
    .command(
      ...
      {
        url: {
          description: 'Base URL of the mlchain server from which you want to pull changes',
          default: 'http://localhost:3000',
          type: 'string'
        },
        authToken: {
          alias: 'token',
          description: 'Authorization token on the remote mlchain server',
          type: 'string'
        },
        targetDir: {
          alias: 'dir',
    ...
    .command(
      'push',
      'Push local files to a remote mlchain server',
      {
        url: {
          description: 'URL of the mlchain server to which to push changes',
          default: 'http://localhost:3000',
          type: 'string'
        },
        authToken: {
    ...
    .command(
      ...
      {
        file: {
          description: 'Complete path of the remote file (ex: global/mlchain.config.json)',
          type: 'string'
        },
        dest: {
          description:
    ...
    .command(
      ...
      {
        file: {
          description: 'Path of the local file (eg: mlchain.config.json)',
          type: 'string'
        },
        dest: {
          description: 'Complete path of the destination file (ex: global/mlchain.config.json)',
          type: 'string'
        }
      },
      argv => {
    ...
    .command(
      ...
      {
        url: {
          description: 'Base URL of the mlchain server you want to benchmark',
          default: 'http://localhost:3000'
        },
        botId: {
    ...
    .command('version', "Display the server's version", {}, () => {
      console.info(`Mlchain: v${metadataContent.version}`)
