import { BotConfig, BotTemplate, Logger, Stage, WorkspaceUserWithAttributes } from 'mlchain/sdk'
...
export class BotService {
  ...
  private async _executeStageChangeHooks(beforeRequestConfig: BotConfig, currentConfig: BotConfig) {
    ...
    const bpConfig = await this.configProvider.getMlchainConfig()
  ...
  private async _localMount(botId: string): Promise<boolean> {
    ...
    try {
      ...
      if (ms(mlchainConfig.dialog.sessionTimeoutInterval) < ms(config.dialog?.timeoutInterval || '0s')) {
        this.logger
          .forBot(botId)
          .warn(
            `[${botId}] Your timeout interval (source: bot.config) is greater than your session timeout (source: mlchain.config). This will prevent 'before_session_timeout' hooks and Timeout flows from being executed.`
          )
      }
