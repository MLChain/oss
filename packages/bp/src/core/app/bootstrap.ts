...
import { MlchainApp, createApp, createLoggerProvider } from 'core/app/core-loader'
...
async function setupEnv(app: MlchainApp) {
...
async function prepareLocalModules(app: MlchainApp, logger: sdk.Logger) {
...
async function start() {
  ...
  if (cluster.isMaster) {
    ...
    if (await isElevated()) {
      logger.warn(
        'You are running Mlchain as a privileged user. This is not recommended. Please consider running it as an unprivileged user.'
      )
    }
  ...
  const globalConfig = await app.config.getMlchainConfig()
  ...
  showBanner({ title: 'Mlchain Server', version: sdk.version, logScopeLength: 9, bannerWidth: 75, logger })
...
Please make sure that Mlchain has the right to access this folder or change the folder path by providing the 'APP_DATA_PATH' env variable.
This is a fatal error, process will exit.`
  ...
  await app.mlchain.start({ modules: loadedModules.map(m => m.entryPoint) }).catch(err => {
    logger.attachError(err).error('Error starting Mlchain')
  ...
  logger.info('-->  Documentation is available at    📘 https://mlchain.com/docs')
  logger.info('-->  Ask your questions on            👥 https://forum.mlchain.com')
  logger.info('='.repeat(75))
  logger.info('')

  logger.info(chalk.bold('Mlchain is ready. open the Studio in your favorite browser.'))
  logger.info(chalk.bold(`Mlchain is listening at ${process.LOCAL_URL} (browser)`))
  logger.info(chalk.bold(`Mlchain is exposed at ${process.EXTERNAL_URL}`))
}
