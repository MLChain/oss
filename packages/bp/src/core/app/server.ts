...
import { AxiosBotConfig, AxiosOptions, http, Logger, RouterOptions } from 'mlchain/sdk'
...
export class HTTPServer {
  ...
  async setupRootPath() {
    const mlchainConfig = await this.configProvider.getMlchainConfig()
    const externalUrl = process.env.EXTERNAL_URL || mlchainConfig.httpServer.externalUrl
  ...
  async initialize() {
    ...
    const mlchainConfig = await this.configProvider.getMlchainConfig()
    process.USE_JWT_COOKIES = yn(mlchainConfig.jwtToken.useCookieStorage)

    const app = express()
    app.use(process.ROOT_PATH, this.app)
    this.httpServer = createServer(app)

    const { keepAliveTimeout } = mlchainConfig.httpServer
    ...
    AppLifecycle.waitFor(AppLifecycleEvents.MLCHAIN_READY).then(() => {
      this.isMlchainReady = true
    })
  }
  ...
  async start() {
    const mlchainConfig = await this.configProvider.getMlchainConfig()
    const config = mlchainConfig.httpServer
    await this.sdkApiRouter.initialize()

    await this.messagingService.proxy.setup(this.app, BASE_API_PATH)

    /**
     * The loading of language models can take some time, access to Mlchain is disabled until it is completed
      ...
      if (!this.isMlchainReady) {
        if (
          ...
          return res
            .status(503)
            .send(
              '<html><head><meta http-equiv="refresh" content="2"> </head><body>Mlchain is loading. Please try again in a minute.</body></html>'
            )
        }
      }
      next()
    ...
    this.app.get('/version', async (req, res) => {
      res.send(process.MLCHAIN_VERSION)
    })
    ...
    this.app.use((err, _req, _res, next) => {
      if (err instanceof UnlicensedError) {
        ...
        if (err.statusCode === 413) {
          this.logger.error('You may need to increase httpServer.bodyLimit in file data/global/mlchain.config.json')
        }
        next(err)
      }
    })
    ...
    this.app.use(function handleUnexpectedError(err, req, res, next) {
      ...
      const docs = err.docs || 'https://mlchain.com/docs'
  ...
  private async _getExternalAuthConfig(): Promise<ExternalAuthConfig | undefined> {
    const mlchainConfig = await this.configProvider.getMlchainConfig()
    const config = mlchainConfig.pro.externalAuth
