...
@injectable()
export class Mlchain {
  mlchainPath: string
  configLocation: string
  modulesConfig: any
  config!: MlchainConfig | undefined
  api!: typeof sdk
  ...
  constructor(
    ...
    this.mlchainPath = path.join(process.cwd(), 'dist')
    this.configLocation = path.join(this.mlchainPath, '/config')
  }
  ...
  private async initialize(options: StartOptions) {
    ...
    this.config = await this.configProvider.getMlchainConfig()
    ...
    AppLifecycle.setDone(AppLifecycleEvents.MLCHAIN_READY)
  ...
  private async maybeStartLocalActionServer() {
    const { actionServers, experimental } = await this.configProvider.getMlchainConfig()
  ...
  private async maybeStartLocalSTAN() {
    if (!process.LOADED_MODULES['nlu']) {
      this.logger.warn(
        'NLU server is disabled. Enable the NLU module and restart Mlchain to start the standalone NLU server'
      )
      return
    }
  ...
  async checkJwtSecret() {
    // @deprecated > 11: .jwtSecret has been renamed for appSecret. mlchain > 11 jwtSecret will not be supported
    // @ts-ignore
    let appSecret = this.config.appSecret || this.config.jwtSecret
    if (!appSecret) {
      appSecret = nanoid(40)
      await this.configProvider.mergeMlchainConfig({ appSecret }, true)
  ...
  async checkEditionRequirements() {
    ...
    if (!process.IS_PRO_ENABLED && process.CLUSTER_ENABLED) {
      this._killServer(
        'Redis is enabled in your Mlchain configuration. To use Mlchain in a cluster, please upgrade to Mlchain Pro.'
      )
    }
    ...
    if (!process.IS_PRO_ENABLED) {
      ...
      if (workspaces.length > 1) {
        this._killServer(
          'You have more than one workspace. To create unlimited workspaces, please upgrade to Mlchain Pro.'
        )
      }
      ...
      if (workspaces.length) {
        for (const workspace of workspaces) {
          ...
          if (pipeline && pipeline.length > 1) {
            this._killServer(
              'Your pipeline has more than a single stage. To enable the pipeline feature, please upgrade to Mlchain Pro.'
            )
          }
        }
      }
    ...
    bots.forEach(bot => {
      if (!process.IS_PRO_ENABLED && bot.languages && bot.languages.length > 1) {
        this._killServer(
          `A bot has more than a single language (${bot.id}). To enable the multilingual feature, please upgrade to Mlchain Pro.`
        )
      }
    })
    if (process.IS_PRO_ENABLED && !process.CLUSTER_ENABLED) {
      this.logger.warn(
        'Mlchain can be run on a cluster. If you want to do so, make sure Redis is running and properly configured in your environment variables'
      )
    }
    if (process.IS_PRO_ENABLED && dbType !== 'postgres' && process.CLUSTER_ENABLED) {
      this._killServer(
        'Postgres is required to use Mlchain in a cluster. Please migrate your database to Postgres and enable it in your Mlchain configuration file.'
      )
    }
    ...
    if (!process.IS_PRO_ENABLED && this.config?.pro.branding) {
      this.logger.warn('Mlchain Pro must be enabled to use a custom theme and customize the branding.')
    }
  }
  ...
  private async initializeServices() {
    ...
    this.configProvider.onMlchainConfigChanged = async (initialHash: string, newHash: string) => {
    ...
    this.decisionEngine.onAfterEventProcessed = async (event: sdk.IO.IncomingEvent) => {
      ...
      completedWorkflows.forEach(async workflow => {
        ...
        MLCHAIN_CORE_EVENT(metric, { botId: event.botId, channel: event.channel, wfName: workflow })
  ...
  private async cleanDisabledModules() {
    try {
      const config = await this.configProvider.getMlchainConfig()
  ...
  private trackStart() {
    this.stats.track(
      'server',
      'start',
      `version: ${process.MLCHAIN_VERSION}, pro: ${process.IS_PRO_ENABLED}, licensed: ${process.IS_LICENSED}`
    )
  }
    ...
    this._heartbeatTimer = setInterval(async () => {
      ...
      try {
        ...
        this.stats.track(
          'server',
          'heartbeat',
          `version: ${process.MLCHAIN_VERSION}, pro: ${process.IS_PRO_ENABLED}, licensed: ${process.IS_LICENSED}, bots: ${nbBots}, collaborators: ${nbCollabs}`
        )
      }
    }, ms('2m'))
  }
