...
const config = (moduleLoader: ModuleLoader, configProvider: ConfigProvider): typeof sdk.config => {
  return {
    ...
    getMlchainConfig: configProvider.getMlchainConfig.bind(configProvider),
...
@injectable()
export class MlchainAPIProvider {
...
export function createForModule(moduleId: string): Promise<typeof sdk> {
  ...
  return container.get<MlchainAPIProvider>(TYPES.MlchainAPIProvider).create(`Mod[${moduleId}]`, `module.${moduleId}`)
}

export function createForGlobalHooks(): Promise<typeof sdk> {
  // return Promise.resolve(<typeof sdk>{})
  return container.get<MlchainAPIProvider>(TYPES.MlchainAPIProvider).create('Hooks', 'hooks')
}

export function createForMlchain(): Promise<typeof sdk> {
  return container.get<MlchainAPIProvider>(TYPES.MlchainAPIProvider).create('Mlchain', 'mlchain')
}

export function createForAction(): Promise<typeof sdk> {
  return container.get<MlchainAPIProvider>(TYPES.MlchainAPIProvider).create('Actions', 'actions')
}
