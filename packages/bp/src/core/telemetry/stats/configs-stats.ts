...
export class ConfigsStats extends TelemetryStats {
  ...
  constructor(
    ...
    this.lock = 'mlchain:telemetry-configs'
    this.interval = ms('7d')
  }
  ...
  protected async getStats(): Promise<TelemetryEvent> {
    const temp = {
      ...
      event_data: {
        ...
        mlchainConfig: await this.getMlchainConfig()
      }
    }
    return temp
  }
  ...
  private async getMlchainConfig(): Promise<MlchainConfig> {
    const mlchainConfig = this.obfuscateModulesLocation(await this.config.getMlchainConfig())
    const defaultConfig = defaultJsonBuilder(await this.fetchSchema('mlchain.config.schema.json'))

    return this.obfuscateSecrets(mlchainConfig, defaultConfig)
  }

  private obfuscateModulesLocation(mlchainConfig: MlchainConfig) {
    const modules = mlchainConfig.modules.map(module => {
