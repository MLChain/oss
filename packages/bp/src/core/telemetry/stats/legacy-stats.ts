...
export class LegacyStats extends TelemetryStats {
  ...
  constructor(
    ...
    this.url = 'https://telemetry.mlchain.io/ingest'
    this.lock = 'mlchain:telemetry-legacyStats'
    this.interval = ms('1d')
  }

  protected async getStats() {
    const config = await this.config.getMlchainConfig()
  ...
  protected async getServerStats() {
    return {
      ...
      mlchainVersion: process.MLCHAIN_VERSION,
