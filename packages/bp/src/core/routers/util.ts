import { Logger, StrategyUser } from 'mlchain/sdk'
  ...
  return multer({
    fileFilter: (_req, file, cb) => {
      ...
      if (!extMimeType) {
        cb(
          new Error(
            `The file has no extension: ${file.originalname}. To allow any kind of files, set the value of "allowedMimeTypes" to "['*']" in your mlchain.config.json file.`
          )
        )
      } else {
