...
import { MlchainAPIProvider } from 'core/app/api'
...
export class MigrationService {
  ...
  constructor(
    ...
    this.targetVersion = process.MIGRATE_TARGET || process.env.TESTMIG_BP_VERSION || process.MLCHAIN_VERSION
  }
  ...
  async initialize() {
    ...
    if (!process.AUTO_MIGRATE) {
      this.logger.error(
        'Mlchain needs to migrate your data. Please make a copy of your data, then start it with "./bp --auto-migrate"'
      )
  ...
  async detectAndSetupVersions(migrations: MigrationFile[]) {
    ...
    this.configVersion = process.env.TESTMIG_CONFIG_VERSION || (await this.configProvider.getMlchainConfig()).version
    ...
    if (process.core_env.TESTMIG_ALL || process.core_env.TESTMIG_NEW) {
      ...
      this.configVersion = yn(process.core_env.TESTMIG_NEW) ? process.MLCHAIN_VERSION : '12.0.0'
    }
  ...
  public async getDbVersion(): Promise<string> {
    ...
    if (process.env.TESTMIG_DB_VERSION) {
      ...
      dbVersion = yn(process.core_env.TESTMIG_NEW) ? process.MLCHAIN_VERSION : '12.0.0'
    } else {
  ...
  async persistMigrationStatus(logs: string[], migrationsToExecute: MigrationFile[]) {
    ...
    // srv_migrations tracks migration of mlchain config, while srv_metadata is for the database
    try {
      await this.database
        .knex('srv_migrations')
...
${_.repeat(' ', 9)}========================================`)
    ...
    // Clear the Mlchain cache before executing any migrations
    try {
    ...
    if (hasFailures) {
      this.logger.error(
        `${logPrefix}Some steps failed to complete. Please fix errors manually, then restart Mlchain so the update process may finish.`
      )
    ...
    if (!process.MIGRATE_DRYRUN) {
      await this.configProvider.mergeMlchainConfig({ version: this.targetVersion })
    }
...
${_.repeat(' ', 9)}========================================`)
  ...
  public async getMigrationOpts(metadata?: sdk.MigrationMetadata) {
    return {
      bp: await container.get<MlchainAPIProvider>(TYPES.MlchainAPIProvider).create('Migration', 'core'),
