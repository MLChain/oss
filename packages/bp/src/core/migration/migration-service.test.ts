...
describe('Migration Service', () => {
  ...
  describe('getDbVersion', () => {
    ...
    const OLD_MLCHAIN_VERSION = process.MLCHAIN_VERSION

    const mock_MLCHAIN_VERSION = 'mocked_version'

    beforeEach(() => {
      jest.resetModules()
      process.env = { ...OLD_ENV }
      process.core_env = { ...OLD_CORE_ENV }
      process.MLCHAIN_VERSION = mock_MLCHAIN_VERSION
    })

    afterEach(() => {
      process.env = OLD_ENV
      process.core_env = OLD_CORE_ENV
      process.MLCHAIN_VERSION = OLD_MLCHAIN_VERSION
    })
    ...
    const testcases: [any, any, string][] = [
      ...
      [undefined, 'no', '12.0.0'],
      [undefined, '0', '12.0.0'],
      [undefined, 'true', mock_MLCHAIN_VERSION],
      [undefined, '1', mock_MLCHAIN_VERSION]
    ]
