...
export class ConverseService {
  ...
  public async sendMessage(
    ...
    if (!maxMessageLength) {
      maxMessageLength = _.get(await this.configProvider.getMlchainConfig(), 'converse.maxMessageLength', 360)
    }
  ...
  private async _createDonePromise(botId, userKey: string) {
    return new Promise((resolve, reject) => {
      converseApiEvents.once(`done.${userKey}`, async event => {
        ...
        if (!bufferDelay) {
          bufferDelay = _.get(await this.configProvider.getMlchainConfig(), 'converse.bufferDelayMs', 250)
        }
  ...
  private async _createTimeoutPromise(botId, userId) {
    ...
    if (!timeout) {
      timeout = _.get(await this.configProvider.getMlchainConfig(), 'converse.timeout', '5s')
    }
