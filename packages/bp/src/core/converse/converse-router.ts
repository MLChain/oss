...
// see https://mlchain.com/docs/build/channels/#usage-public-api
...
export class ConverseRouter extends CustomRouter {
  ...
  private shouldRespondToPublicEndpoint = async (req, res, next) => {
    ...
    const enabledGlobal = (await this.configProvider.getMlchainConfig()).converse?.enableUnsecuredEndpoint ?? true
    if (!enabledGlobal) {
