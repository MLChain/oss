...
export class ModuleLoader {
  ...
  private async callModulesOnReady(modules: ModuleEntryPoint[], initedModules: {}): Promise<void> {
    ...
    // TODO We probably want to wait until Mlchain is done loading the other services etc
    for (const module of modules) {
  ...
  public async getAllModules(): Promise<ModuleInfo[]> {
    const configModules = (await this.configProvider.getMlchainConfig()).modules
