import { ConverseConfig } from 'mlchain/sdk'
...
export type MlchainCondition = '$isProduction' | '$isDevelopment'
...
export interface MlchainConfig {
  version: string
  appSecret: string
  httpServer: {
    /**
    ...
    port: number
    /**
     * There are three external URLs that Mlchain calls: https://license.mlchain.io, https://duckling.mlchain.io and https://lang-01.mlchain.io
  ...
  pro: {
     ...
     * You can purchase a license on https://mlchain.com
     * For usage with Mlchain Pro/Enterprise.
     ...
     * data about the user that you would like to make Mlchain aware of.
     *
  ...
  /**
   * An array of e-mails of users which will have root access to Mlchain (manage users, server settings)
   * @example: [admin@mlchain.com]
   */
  superAdmins: UniqueUser[]
  /**
   * When enabled, Mlchain collects anonymous data about the bot's usage
   * @default true
   */
  ...
  /**
   * An array of categories in which a bot can be created in your mlchain installation. Good for platform use-cases.
   * @example: ['customer service', 'e-commerce', 'etc']
   * @default []
   */
  botCategories: string[]
  /**
   * When this option is enabled, Super Admins are able to reboot the Mlchain Server via the Admin UI
  ...
  jwtToken: {
    /**
     * The duration for which the token granting access to manage Mlchain will be active.
     * @default 1h
     */
    duration: string
    /**
  ...
  /**
   * Displays the "Powered by Mlchain" under the webchat.
  ...
  /**
   * By adding this, you'll make possible to translate a bot in more languages than those supported by your mlchain language server
   * Warning: This means that Mlchain NLU won't be working properly and you'll need to handle NLU on your own with a **beforeIncoming** Hook.
...
export interface AuthStrategy {
  ...
  /**
   * Set a label to display to users instead of the ID (ex: Mlchain SSO)
   */
  label?: string
  /**
  ...
  /**
   * Maps the values returned by your provider to Mlchain user parameters.
   ...
   * any user able to sign in using your SAML provider will create automatically an account on Mlchain.
   * @default false
   */
...
export interface AuthStrategyOauth2 {
  ...
  callbackURL: string
  /*
   * Set this URL if your access token doesn't include user data. Mlchain will query that URL to fetch user information
   * @example https://mlchain.com/userinfo
   */
  userInfoURL?: string
