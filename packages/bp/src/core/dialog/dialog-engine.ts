import { FlowNode, IO, Logger } from 'mlchain/sdk'
...
export class DialogEngine {
  ...
  public changeWorkflow(event: IO.IncomingEvent, nextFlow: string) {
    ...
    if (!workflow) {
      MLCHAIN_CORE_EVENT('bp_core_workflow_started', { botId: event.botId, channel: event.channel, wfName: nextFlow })
  ...
  protected async _transition(sessionId: string, event: IO.IncomingEvent, transitionTo: string) {
    ...
    if (transitionTo.includes('.flow.json')) {
      MLCHAIN_CORE_EVENT('bp_core_enter_flow', { botId: event.botId, channel: event.channel, flowName: transitionTo })
