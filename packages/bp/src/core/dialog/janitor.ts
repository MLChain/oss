import { BotConfig, IO, Logger } from 'mlchain/sdk'
...
export class DialogJanitor extends Janitor {
   ...
   * NOTE: This task will run all mlchain server nodes and we want only one node to process session timeouts,
  ...
  private async _processSessionTimeout(sessionId: string, botId: string, botConfig: BotConfig) {
    ...
    const bpConfig = await this.getMlchainConfig()
  ...
  private async _updateState(botId: string, botConfig: BotConfig, sessionId: string, update: updateArgs) {
    ...
    const expiry = createExpiry(botConfig!, mlchainConfig)
    const session = await this.sessionRepo.get(sessionId)
  ...
  public async detectTimeoutInfiniteLoop(session: DialogSession, botId: string) {
    try {
      ...
      const limit = 5
      const dialogTimeout = _.get(botConfig, 'dialog.timeoutInterval', mlchainConfig.dialog.timeoutInterval) as string
