import { IO, Logger, NDU } from 'mlchain/sdk'
...
export class DecisionEngine {
  ...
  async initialize() {
    ...
    this.noRepeatPolicy = (await this.configProvider.getMlchainConfig()).noRepeatPolicy
  }
  ...
  private async processEventNDU(sessionId: string, event: IO.IncomingEvent) {
    ...
    for (const { action, data } of event.ndu.actions) {
      if (action === 'send' && data) {
        ...
        MLCHAIN_CORE_EVENT('bp_core_send_content', {
          botId: event.botId,
