...
export const assertMlchainPro = (workspaceService: WorkspaceService) => async (
  ...
  if (!process.IS_PRO_ENABLED || !process.IS_LICENSED) {
    ...
    if ((await workspaceService.getUniqueCollaborators()) > 0) {
      return next(new PaymentRequiredError('Mlchain Pro is required to perform this action'))
    }
  }
