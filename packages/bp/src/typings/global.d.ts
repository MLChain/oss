declare namespace NodeJS {
  ...
  export interface Global {
    ...
    DEBUG: IDebug
    MLCHAIN_CORE_EVENT: IEmitCoreEvent
    MLCHAIN_CORE_EVENT_TYPES: MlchainCoreEvents
  ...
  export interface Process {
    ...
    APP_SECRET: string
    /**
     * Path to the global APP DATA folder, shared across all installations of Mlchain Server
    ...
    EXTERNAL_URL: string
    LOCAL_URL: string
    /** This is the subfolder where Mlchain is located (ex: /mlchain/). It is extracted from the external URL */
    ROOT_PATH: string
    ...
    core_env: MlchainEnvironmentVariables
    distro: OSDistribution
    MLCHAIN_EVENTS: EventEmitter
 ...
 */
declare interface MlchainEnvironmentVariables {
  ...
  /** The URL exposed by Mlchain to external users (eg: when displaying links) */
  ...
  /** Use this to override the hostname that mlchain will listen on (by default it's localhost) - replaces httpServer.host */
  readonly BP_HOST?: string

  /** Change the port where mlchain listens. Replaces the configuration of httpServer.port */
  ...
  /**
   * The scope or channel prefix used by RedisIO to differentiate multiple clusters of Mlchain using the same Redis Cluster.
   * See: https://redis.io/topics/pubsub#database-amp-scoping
   * @example production, staging, test, development, mlchain1, ...
   */
  ...
  /** When running mlchain in production, some optimizations are applied */
  readonly BP_PRODUCTION?: boolean

  /** Enable cluster mode */
  readonly CLUSTER_ENABLED?: boolean

  /** When you change the mlchain executable, it will migrate data automatically if this is set */
   ...
   * @default https://license.mlchain.io
   */
  readonly BP_LICENSE_SERVER_HOST?: string

  /**
   * Set this to true if you're exposing Mlchain through a reverse proxy such as Nginx
   ...
   *  This values overwrites the value defined in the global Mlchain configuration
  ...
  /**
   * Truthy if running the official Mlchain docker image
   */
  ...
  /**
   * When set to true, Mlchain will not automatically restart on crash
   * @default false
   */
  ...
  /**
   * Prevents Mlchain from closing cleanly when an error is encountered.
...
declare interface MlchainCoreEvents {
...
interface IEmitCoreEvent {
  <T extends keyof MlchainCoreEvents>(
    event: T,
    args: { [key in keyof MlchainCoreEvents[T]]: MlchainCoreEvents[T][key] }
  ): void
}

declare var MLCHAIN_CORE_EVENT: IEmitCoreEvent
