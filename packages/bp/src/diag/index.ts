...
import { MlchainApp, createApp } from 'core/app/core-loader'
...
const REDIS_TEST_KEY = 'mlchain_redis_test_key'
const REDIS_TEST_VALUE = nanoid()

let app: MlchainApp
let redisClient: IORedis.Redis
let includePasswords = false
let mlchainConfig = undefined
...
const printGeneralInfos = async () => {
  printHeader('General')
  printRow('Mlchain Version', process.MLCHAIN_VERSION)
...
const testConnectivity = async () => {
  ...
  try {
    ...
    printRow('Mlchain nodes listening on Redis', reply[1])
  } catch (err) {}
...
const testNetworkConnections = async () => {
  ...
  const hosts = [
    ...
    },
    {
      ...
      url: `https://${process.env.BP_LICENSE_SERVER_HOST || 'license.mlchain.io'}/prices`
    }
  ]
...
const printConfig = async () => {
  printHeader('Mlchain Config')
  printObject(mlchainConfig, includePasswords)
...
export default async function(options: Options) {
  ...
  try {
    ...
    mlchainConfig = await app.ghost.global().readFileAsObject<any>('/', 'mlchain.config.json')
  } catch (err) {}
  ...
  if (options.monitor || yn(process.env.BP_DIAG_MONITOR)) {
    await startMonitor(mlchainConfig!, redisClient)
