  ...
  async setupRoutes() {
    ...
    this.router.post(
      ...
      this.asyncMiddleware(async (req, res) => {
        ...
        if (featureId === 'pro') {
          await this.configProvider.mergeMlchainConfig({ pro: { enabled: true } })
        } else if (featureId === 'monitoring') {
          await this.configProvider.mergeMlchainConfig({ pro: { monitoring: { enabled: true } } })
        } else if (featureId === 'alerting') {
          await this.configProvider.mergeMlchainConfig({ pro: { alerting: { enabled: true } } })
        }
    ...
    this.router.post(
      ...
      this.asyncMiddleware(async (req, res) => {
        ...
        if (!config.allowServerReboot) {
          ...
          return res.status(400).send('Rebooting the server is disabled in the mlchain.config.json file')
        }
