  ...
  setupRoutes() {
    ...
    router.post(
      ...
      this.asyncMiddleware(async (req, res) => {
        ...
        // We want to update the licenseKey in mlchain.config.json if the user manually replaces its key
        const pro = {
          enabled: process.IS_PRO_ENABLED,
          licenseKey: req.body.licenseKey
        }
        await this.configProvider.mergeMlchainConfig({ pro })
    ...
    router.post(
      ...
      this.asyncMiddleware(async (req, res) => {
        if (process.IS_PRO_ENABLED) {
          return res.send('Mlchain Pro is already enabled.')
        }
