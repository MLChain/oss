...
export const fetchLatestVersions = (): AppThunk => {
  return async (dispatch, getState) => {
    ...
    try {
      const { data } = await axios.get('https://api.github.com/repos/mlchain/oss/releases')
      ...
      try {
        ...
        releases.forEach(r => {
          ...
          if (details) {
            r.dockerUrl = `https://hub.docker.com/layers/mlchain/server/${details.name}/images/${details.hash}`
          }
        })
      } catch (err) {
