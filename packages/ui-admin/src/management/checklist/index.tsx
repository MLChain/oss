...
const Container = props => {
  return (
    <PageContainer
      ...
      helpText={
        <span>
          This is a checklist of recommended settings when running Mlchain in production.
          <br /> Environment variables are displayed in <Tag>gray</Tag> and values from the mlchain.config.json config
    ...
    <Container>
      <div className={style.checklist}>
        <Item
          ...
          docs="https://mlchain.com/docs/building-chatbots/developers/database#how-to-switch-from-sqlite-to-postgressql"
          status={getEnv('DATABASE_URL').startsWith('postgres') ? 'success' : 'warning'}
          source={[{ type: 'env', key: 'DATABASE_URL', value: '**********' }]}
        >
          By default, Mlchain uses an SQLite database, which is not recommended in a production environment. Postgres
          is more resilient and allows to run Mlchain in cluster mode (using multiple servers to handle the load).
        </Item>

        <Item
          title="Use the database BPFS storage"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#use-the-database-bpfs-storage"
        ...
        <Item
          title="Run Mlchain in production mode"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#run-mlchain-in-production-mode"
          status={getEnv('BP_PRODUCTION') === 'true' ? 'success' : 'warning'}
          source={[{ type: 'env', key: 'BP_PRODUCTION', value: getEnv('BP_PRODUCTION') }]}
        >
          When you run Mlchain in production, these changes happens:
          <ul>
        ...
        <Item
          ...
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#configure-the-external-server-url"
        ...
        >
          <span>
            ...
            When it is not set, it defaults to http://localhost:3000. When using Mlchain Professional, this value is
        ...
        >
          Redis allows you to run multiple Mlchain servers, all using the same data. Only 'REDIS_URL' and
          'CLUSTER_ENABLED' are required for Redis to work properly. Setting a Redis scope allows you to run multiple
          Mlchain clusters (e.g. staging and production) on the same Redis cluster without impacting one another (not
        ...
        >
          By default, Mlchain allows any origin to reach the server. You can either disable CORS completely (set the
          configuration to false), or set an allowed origin
        </Item>

        <Item
          title="Enable Cookie storage for the JWT Token"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#enable-cookie-storage-for-the-jwt-token"
        ...
        <Item
          ...
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#host-your-own-language-server"
          status={languageEndpoint.includes('mlchain.io') ? 'warning' : 'success'}
          source={[{ type: 'config', key: 'nlu.json: languageSources', value: languageEndpoint }]}
        >
          The default language server configured with Mlchain is a public server, which has request limitations and
        ...
        <Item
          ...
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#securing-your-server-with-https"
          status={protocol === 'https' ? 'success' : 'warning'}
          source={[{ key: 'Detected protocol', value: protocol }]}
        >
          Mlchain doesn't handle certificates and https headers directly. Those should be handled by a NGINX server in
          front of it. We have a recommended NGINX configuration sample in the documentation.
        </Item>

        <Item
          title="Enable audit trail"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#enable-audit-trail"
        ...
        <Item
          ...
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#enable-sticky-sessions"
          status="none"
          source={[
        ...
        <Item
          ...
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#output-logs-to-the-filesystem"
        ...
        >
          By default, Mlchain does some minimal logging to the database. It is recommended to enable the log output on
          the file system to keep traces
        </Item>

        <Item
          title="Change Mlchain base path"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#change-mlchain-base-path"
        ...
        >
          ...
          (for example to use http://localhost:3000/mlchain). You can do that by updating your server's EXTERNAL_URL
          and adding the suffix at the end.
        </Item>

        <Item
          title="Create custom roles and review permissions"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#create-custom-roles-and-review-permissions"
          status="none"
        >
        ...
        <Item
          ...
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#enable-other-authentication-mechanism"
          status="none"
        >
          The default authentication method is a username/password, but you can enable additional authentication
          strategies to access Mlchain. We currently support LDAP, SAML and OAUTH2.
        </Item>

        <Item
          title="Configure your Reverse Proxy and Load Balancing"
          docs="https://mlchain.com/docs/enterprise/server-and-cicd-management/production-checklist#configure-your-reverse-proxy-and-load-balancing"
          status="none"
        >
