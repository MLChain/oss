...
You can link your SAML Identity Provider seamlessly with Mlchain. When it is enabled, Admins will be greeted with a `Sign in with SSO` button on the Admin UI. The first user to ever log in to Mlchain using the SSO provider will automatically have an account created and a Super Admin.

When a user successfully logs on to the Admin UI, Mlchain will create an internal account for that user. Mlchain will add them to the table `strategy_STRATEGYID`
...
- Mlchain Enterprise enabled with a valid license key
...
1. Open `mlchain.config.json` and set `pro.auth.strategy = 'saml'`
...
"auth": {
  "strategy": "saml",
  "options": {
    "entryPoint": "https://mlchain-saml-idp.auth0.com/somestuff/bla",
    "callbackUrl": "http://mlchain.yourdomain.net/api/v1/auth/login-callback/saml/saml",
    "path": "http://mlchain.yourdomain.net/api/v1/auth/login-callback/saml/saml",
    "issuer": "mlchain-saml",
