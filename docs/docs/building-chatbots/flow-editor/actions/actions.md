...
`Libraries` is deprecated and will be removed in Mlchain 13.
::: -->
...
If you need to use npm modules / libraries or libraries from other programming languages, we recommend you either set up a server for that purpose, or use serverless functions and then call the api using Mlchain Actions. Using Mlchain for heavy javascript computation use cases like image / video processing may cause latency issues for your bot. 

## Disabling Actions

Mlchain will ignore files starting with a dot (`.`). This way, you can disable a hook or Action by merely prefixing the file's name with a dot.
...
This action retrieves a variable that was stored globally using a storage key. Mlchain uses a `key: value` storage system to allow complex object storage definitions.
...
Mlchain comes pre-packed with a translation engine that helps developers design a bot in one language while catering to users of all supported and configured languages.
...
function action(bp: typeof sdk, event: sdk.IO.IncomingEvent, args: any, { user, temp, session } = event.state) {
   ...
   *     actions: [{ title: 'Mlchain website Documentation', action: 'Open URL', url: 'https://mlchain.com/docs' }]
   *   }
   * ```
...
function action(bp: typeof sdk, event: sdk.IO.IncomingEvent, args: any, { user, temp, session } = event.state) {
   ...
   *         subtitle: 'Website',
   *         image: 'https://avatars.githubusercontent.com/u/23510677?s=200&v=4',
   *         actions: [{ title: 'Mlchain website', action: 'Open URL', url: 'https://mlchain.com' }]
   *       }
   *     ],
   *     typing: true
   *   },
...
Mlchain can display PDF file. Currently, we are only supporting PDF.
