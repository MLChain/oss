...
:::note
Unlike the subject field, you can use markdown, which Mlchain will render correctly to the receiver. You can also include HTML to enhance the formatting of your email body further.
:::

Mlchain supports templating in all fields of your email skill, allowing access to variables stored in Memory. All `bot`, `user`, `session`, `temp`, and `event` are accessible via templating.
...
You can edit those variable in Mlchain from the **Code Editor**.
...
:::note Notes
- Setting `"rejectUnauthorized": false` will prevent the Mlchain server from rebooting every time an email fails to send. We recommend a fall-back strategy if this happens using the `on failure` transition.
