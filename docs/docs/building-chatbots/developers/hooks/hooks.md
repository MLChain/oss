...
Hooks have access to the [Mlchain SDK](https://mlchain.com/reference/).

## Disabling Hooks

Mlchain will ignore files starting with a dot (`.`). This way, you can disable a hook or action by prefixing the file's name with a dot.
...
Every time a bot is unmounted, Mlchain calls this event to clean up after deleting a bot.
...
Mlchain calls this hook after receiving an event but before processing any middleware. It is possible to change event properties.

Location: `data/global/hooks/before_incoming_middleware`

Parameters: `bp`, `event`

Mlchain often uses this hook to set flags to skip some processing, for example, to prevent the module QNA from processing it when it's a quick reply:
...
A typical operation here is to tell Mlchain to ignore the event and not process it (e.g., not sending it to the dialog engine).
