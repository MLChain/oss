...
Mlchain has added flexibility for developers who want access to the core codebase. You can clone Mlchain from the source repository on Github, allowing you to test code, modules, and components more dynamically.

## Compiling From Source

You can build Mlchain from the [source repository](https://github.com/mlchain/oss) in a few simple steps. Doing this is useful when you need to create custom modules and components.
...
While in the directory where you want to host your instance of Mlchain, run the following commands in this sequence:

1. `git clone git@github.com:mlchain/oss.git && cd mlchain`
...
1. Go to the [Releases](https://github.com/mlchain/oss/releases) page.
...
You might run into issues while trying to build and start mlchain via yarn on Rasberry Pi OS x64 or other Ubuntu Systems. Its ARM Architecture means none of the pre-built binaries will work. On trying to run the command `yarn start`, you might run into an error like the one below:

```bash
yarn start
yarn run v1.22.10
$ cd ./out/bp && cross-env NODE_PATH=./ cross-env BP_MODULES_PATH=./data/modules/:../../modules:../../internal-modules node index.js
Error starting mlchain
...
RUN git clone https://github.com/mlchain/node-fasttext.git .
RUN git submodule update --init && sh linux-build.sh && npm install && npm run-script build

WORKDIR /build/node-crfsuite
RUN git clone https://github.com/mlchain/node-crfsuite.git .
RUN git submodule update --init && npm install && npm run-script build

WORKDIR /build/node-svm
RUN git clone https://github.com/mlchain/node-svm.git .
RUN git submodule update --init && npm install && npm run-script build

WORKDIR /build/node-sentencepiece
RUN git clone https://github.com/mlchain/node-sentencepiece.git .
RUN git submodule update --init && npm install && npm run-script build
...
If you’re running Mlchain from sources, the correct location would either be: `build/native-extensions/linux/default or create` the directory `build/native-extensions/linux/<your-distribution>`. You can look at the file `rewire.ts` if you want to see how the important processes occur.

If you’re using the Mlchain official binary, place the files in a directory named `bindings`.

After following the instructions above, you're good to go.

## Config File

Mlchain uses `JSON` files for most configurations. Environment variables can also set configuration.

## Mlchain Global Config

The Mlchain global config file is the main file used to configure the Mlchain server. Your instance of Mlchain creates this file automatically if it is missing. Default values should work well when using Mlchain, but we will show you other configurations you may need to change on this page.

To get more information about each option, check out the [comments on the configuration schema](https://github.com/mlchain/oss/blob/master/src/bp/core/config/mlchain.config.ts)

## HTTP Server Configuration

By default, Mlchain starts an HTTP server on localhost, listening to port 3000. If the configured port is already in use, it will pick the next available one. You can change these by editing `httpServer.host` and `httpServer.port`.

### Going to Production

When going to production and publishing your chatbot, you will need to change some of the server settings.

The server doesn't support HTTPS connections, so you will need to set up a reverse proxy in front of it (for example: NGINX). In the main [repo](https://github.com/mlchain/oss), Mlchain have created an example with a HTTPS with [docker-compose](https://github.com/mlchain/oss/blob/master/examples/docker-compose/docker-compose-community-nginx-https.yaml).
...
At this point, Mlchain doesn't know how to access the bot from the web. You need to set the configuration variable to the complete host name (URL) in `httpServer.externalUrl`.

### Changing the Base URL of Your Bot

By default, Mlchain is accessible at your domain root (ex: https://bot.mlchain.com/). You can change that to serve it from a different URL, for example `https://bot.mlchain.com/mlchain/somepath/`. To do so, set the External URL either in environment variable (`EXTERNAL_URL`) or via the `mlchain.config.json` file.
...
When you start Mlchain from the binary (or using the Docker image), the bot is in `debug` mode. This means that a lot of information will be displayed in the console to understand what happens.
...
By default, Mlchain uses the `Debug` configuration.

:::note
When you run Mlchain in production `BP_PRODUCTION=true` or with cluster mode `CLUSTER_ENABLED=true`, logs are configured as `Production`.
:::
...
Edit your `mlchain.config.json` file and change your file to match the following:
...
DEBUG=bp:audit:* ./bp -p 2>> ./mlchain.log
```
...
When you start Mlchain for the first time, the most popular modules included with the binary will be added to your `mlchain.config.json` file. If you want to disable or enable modules, you may either do so in the Admin, or edit the `modules` property in `mlchain.config.json`.
...
When you enable a module on Mlchain, it is available globally, which means that you can't disable or enable them for specific bots. However, you can configure every bot differently.
