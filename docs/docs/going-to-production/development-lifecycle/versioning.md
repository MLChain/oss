...
Once your bot is deployed, the good part is that you (and non-technical team members) can still make changes to your bots from Mlchain Conversation Studio. This is one major advantage of using Mlchain. This is made possible by our built-in versioning system.

For your convenience Mlchain provides the GUI tools to edit these files while in development. We also provide the same tools in production, but there's a caveat. Writing changes to the server's file system is not always possible, they could easily be lost due to ephemeral filesystems or they could be ignored when running in a cluster setup.
...
You can also head to the versioning tab of your mlchain admin panel at https://your.bp.ai/admin/server/version, the command will be properly formatted for you (including your token) any changes have been made. Just paste it to your shell and the changes will be extracted in the provided target directory. A successful output should look like the following:
