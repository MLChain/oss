...
To connect to Microsoft Teams, an HTTPS endpoint is required. This is set in the `externalUrl` field in `mlchain.config.json`. You can use the following methods to create this endpoint:
...
You can also fill the other fields as you wish, but they will not impact Mlchain.
...
1. Start Mlchain, then you should see your Messaging Endpoint in the startup logs.
