...
const onBotMount = async (bp: typeof sdk, botId) => {
  ...
  // We use process variables that are set on server start from mlchain config file, we could use the configs
  // const bpConfig = await bp.config.getMlchainConfig()
