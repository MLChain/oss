...
const repositories = ['mlchain', 'messaging', 'studio', 'nlu']
...
export class WorkspaceManager {
  ...
  initializeWorkspace = async ({ usePro }: { usePro: boolean }) => {
    ...
    for (const repo of Object.keys(config)) {
      await this.setupRepository(repo, { cloneRepo: true, buildAsPro: repo === 'mlchain' && usePro })
    }
  ...
  async syncWorkspace({ forceCheckout, devMode }: { forceCheckout: boolean; devMode: boolean }) {
    ...
    const packageJson = await fse.readJson(path.resolve(this.rootFolder, 'mlchain/package.json'))
  ...
  setupRepository = async (repo: string, { cloneRepo, buildAsPro, forceCheckout, checkoutBranch }: SetupRepository) => {
    ...
    if (cloneRepo) {
      await this.runCommand({
        ...
        cmd: `git clone https://github.com/mlchain/${repo}.git`,
        cwd: this.rootFolder
      })
    }
  ...
  async createEmptyProFile() {
    const proFile = path.resolve(this.rootFolder, 'mlchain/pro')
