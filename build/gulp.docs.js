...
const buildRef = () => {
  return gulp.src(['./packages/bp/src/sdk/mlchain.d.ts']).pipe(
    typedoc({
...
const alterReference = async () => {
  ...
  const original = fs.readFileSync(path.join(__dirname, '../docs/reference/public/modules/_mlchain_sdk_.html'), 'utf8')
  ...
  $('head > title')
    .replaceWith('<title>Chatbot Framework SDK | Mlchain SDK</title>')
    .html(html)

  $('head').append(
    '<meta name="description" content="Mlchain Chatbot software development kit has all the tools you need to create your own custom chatbot.">'
  )
  ...
  fs.writeFileSync(path.join(__dirname, '../docs/reference/public/modules/_mlchain_sdk_.html'), newFile)
  ...
  $('a').map(function() {
    ...
    if (href.startsWith('_mlchain_sdk')) {
