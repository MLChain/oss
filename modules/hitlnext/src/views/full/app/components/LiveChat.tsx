...
const LiveChat: React.FC<Props> = ({ handoff, currentAgent }) => {
  ...
  const webchatEventListener = (message: MessageEvent) => {
    ...
    if (message.data.name === 'webchatLoaded') {
      window.mlchainWebChat.sendEvent({ type: 'show' }, WEBCHAT_ID)
  ...
  useEffect(() => {
    ...
    window.mlchainWebChat.init(webchatConfig, `#${WRAPPER_ID}`)
  }, [webchatLoaded])
  ...
  useEffect(() => {
    ...
    window.mlchainWebChat.sendEvent({ type: 'loadConversation', conversationId: handoff.agentThreadId }, WEBCHAT_ID)
  }, [handoff])
