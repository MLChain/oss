...
export default async (bp: typeof sdk, db: Database, interactionsToTrack: string[]) => {
  ...
  process.MLCHAIN_EVENTS.on('bp_core_send_content', ({ channel, botId, source, details }) => {
    if (source === 'qna') {
      db.incrementMetric(botId, channel, 'msg_sent_qna_count', details)
    }
  })

  process.MLCHAIN_EVENTS.on('bp_core_session_created', ({ channel, botId }) => {
    db.incrementMetric(botId, channel, 'sessions_count')
  })

  process.MLCHAIN_EVENTS.on('bp_core_enter_flow', ({ channel, botId, flowName }) => {
    db.incrementMetric(botId, channel, 'enter_flow_count', removeExt(flowName))
  })

  process.MLCHAIN_EVENTS.on('bp_core_workflow_started', ({ channel, botId, wfName }) => {
    db.incrementMetric(botId, channel, 'workflow_started_count', removeExt(wfName))
  })

  process.MLCHAIN_EVENTS.on('bp_core_workflow_completed', ({ channel, botId, wfName }) => {
    db.incrementMetric(botId, channel, 'workflow_completed_count', removeExt(wfName))
  })

  process.MLCHAIN_EVENTS.on('bp_core_workflow_failed', ({ channel, botId, wfName }) => {
    db.incrementMetric(botId, channel, 'workflow_failed_count', removeExt(wfName))
  })

  process.MLCHAIN_EVENTS.on('bp_core_feedback_positive', ({ channel, botId, type, details }) => {
    if (type === 'qna') {
  ...
  process.MLCHAIN_EVENTS.on('bp_core_feedback_negative', ({ channel, botId, type, details }) => {
    if (type === 'qna') {
