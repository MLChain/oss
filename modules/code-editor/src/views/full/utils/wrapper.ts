...
const wrapper = {
  add: (file: EditableFile, content: string) => {
    ...
    if (type === 'action_legacy' && isJs) {
      ...
      return content.replace('../mlchain.config.schema.json', 'bp://types/mlchain.config.schema.json')
  ...
  remove: (content: string, type: string) => {
    ...
    if (type === 'main_config') {
      return content.replace('bp://types/mlchain.config.schema.json', '../mlchain.config.schema.json')
    }
