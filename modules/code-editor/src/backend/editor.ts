...
export default class Editor {
  ...
  async readFile(name: string, filePath: string) {
    ...
    try {
      ...
      if (name === 'mlchain.d.ts') {
        fileContent = fileContent.replace("'mlchain/sdk'", 'sdk')
      }
    } catch (err) {
  ...
  async loadTypings() {
    ...
    const mlchainConfigSchema = await ghost.readFileAsString('/', 'mlchain.config.schema.json')

    const moduleTypings = await this.getModuleTypings()

    const files = [
      { name: 'node.d.ts', location: path.join(__dirname, '/../typings/node.d.txt') },
      { name: 'mlchain.d.ts', location: path.join(__dirname, '/../mlchain.d.js') },
    ...
    this._typings = {
      ...
      'mlchain.config.schema.json': mlchainConfigSchema,
      ...localTypings,
      ...moduleTypings
    }
